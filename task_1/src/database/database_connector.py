import psycopg2
from src.database.database_queries import create_cart_table, \
    create_item_table, create_cart, create_item, update_item, get_item
...
class DatabaseConnector:
    def __init__(self, db_name, db_user, password, host, init_db=True):
        self.connection = psycopg2.connect(
            ...
            .format(db_name, db_user, password, host))
    ...
    def create_item(self, external_id, name, value, cart_id):
        self.cursor.execute(create_item.format(external_id,
                                               name, value,
                                               cart_id))

    def update_item(self, external_id, name, value, cart_id):
        self.cursor.execute(update_item.format(name, value,
                                               external_id,
                                               cart_id))

    def get_item(self, external_id, cart_id):
        self.cursor.execute(get_item.format(external_id,
                                            cart_id))
